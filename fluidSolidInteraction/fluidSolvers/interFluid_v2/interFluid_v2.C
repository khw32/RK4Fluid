...
defineTypeNameAndDebug(interFluid_v2, 0);
addToRunTimeSelectionTable(fluidSolver, interFluid_v2, dictionary);


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

interFluid_v2::interFluid_v2(const fvMesh& mesh)
:
...
const volVectorField& interFluid_v2::U() const
{
    return U_;
}


const volScalarField& interFluid_v2::p() const
{
    return p_;
}
...
tmp<vectorField> interFluid_v2::patchViscousForce(const label patchID) const
{
    tmp<vectorField> tvF
    (
...
tmp<scalarField> interFluid_v2::patchPressureForce(const label patchID) const
{
    tmp<scalarField> tpF
    (
...
//- Patch viscous force (N/m2)
tmp<vectorField> interFluid_v2::faceZoneViscousForce
(
    const label zoneID,
    const label patchID
) const
...
//- Patch pressure force (N/m2)
tmp<scalarField> interFluid_v2::faceZonePressureForce
(
    const label zoneID,
    const label patchID
) const
...
tmp<scalarField> interFluid_v2::faceZoneMuEff
(
    const label zoneID,
    const label patchID
) const
...
# include "CourantNo_interFluid_v2.H"

    // Make the fluxes absolute to the mesh motion
    // defined in fvcMeshPhi.H
        fvc::makeAbsolute(phi_, U_);  

# include "volContinuityErr_interFluid_v2.H"


# include "correctPhi_interFluid_v2.H"
...
#	include "alphaEqnSubCycles_interFluid_v2.H"
   
#	include "UEqn_interFluid_v2.H"
