...
namespace Foam
{
    defineTypeNameAndDebug(RK4fluidSolver, 0);
    defineRunTimeSelectionTable(RK4fluidSolver, dictionary);
}

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::RK4fluidSolver::calcGlobalFaceZones() const
{
    ...
    if (globalFaceZonesPtr_)
    {
        FatalErrorIn
        (
            "void RK4fluidSolver::calcGlobalFaceZones() const"
        )
...
void Foam::RK4fluidSolver::calcGlobalToLocalFaceZonePointMap() const
{
    // Find global face zones
    if (globalToLocalFaceZonePointMapPtr_)
    {
        FatalErrorIn
        (
            "void RK4fluidSolver::calcGlobalToLocalFaceZonePointMap() const"
        )
...
//         }
        ...
        forAll(curMap, globalPointI)
        {
            if (curMap[globalPointI] == -1)
            {
                FatalErrorIn
                (
                    "RK4fluidSolver::calcGlobalToLocalFaceZonePointMap()"
                )
...
Foam::RK4fluidSolver::RK4fluidSolver
(
    const word& type,
    const fvMesh& mesh
)
...
Foam::RK4fluidSolver::globalFaceZones() const
{
    if (!globalFaceZonesPtr_)
    {
        calcGlobalFaceZones();
...
Foam::RK4fluidSolver::globalToLocalFaceZonePointMap() const
{
    if (!globalToLocalFaceZonePointMapPtr_)
    {
...
Foam::tmp<Foam::vectorField> Foam::RK4fluidSolver::faceZoneVelocity
(
    const label zoneID,
    const label patchID
) const
